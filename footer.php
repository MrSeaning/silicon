...
<script>
    window.onload = function() {
        ...
        <?php if ($this->is("index") || $this->is("post")) : ?>
            ...
            if (page) {
                page.querySelectorAll("li").forEach(function(val, index, arr) {
                    val.classList.add("page-item");
                    val.querySelectorAll("a").forEach(function(val1, index1, arr1) {
                        val1.classList.add("page-link");
                    })
                })
            }
        <?php endif ?>
